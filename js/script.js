const topics = [
    {
        ...
        examples: [
            { desc: "Definición de atributos y métodos básicos:", code: "public class Reloj {\n    String marca;\n    void sonarAlarma() {\n        System.out.println(\"Bip Bip!\");\n    }\n}" },
            { desc: "Instanciación y acceso a miembros:", code: "Reloj miReloj = new Reloj();\nmiReloj.marca = \"Casio\";\nmiReloj.sonarAlarma();" }
        ],
        exercises: [
            { instruction: "Crea una clase 'Laptop' con marca y un método 'procesar()'.", codeChallenge: "public class Laptop {\n    // Escribe aquí\n}" },
            { instruction: "Instancia dos laptops y asígnales marcas diferentes.", codeChallenge: "// Código aquí" }
        ]
    },
    {
        id: 2,
        ...
        examples: [
            { desc: "Uso de private y métodos de acceso:", code: "private double saldo;\npublic double getSaldo() {\n    return saldo;\n}" },
            { desc: "Validación lógica en un Setter:", code: "public void setEdad(int e) {\n    if(e > 0) this.edad = e;\n}" }
        ],
        exercises: [
            { instruction: "Crea un atributo privado 'nombre' con su Getter y Setter.", codeChallenge: "private String nombre;\n// Implementa métodos" },
            { instruction: "Implementa un Setter para 'precio' que rechace valores menores a 1.", codeChallenge: "public void setPrecio(double p) { ... }" }
        ]
    },
    {
        id: 3,
        ...
        examples: [
            { desc: "Extensión de funcionalidad básica:", code: "class Vehiculo { void mover() {} }\nclass Avion extends Vehiculo { void volar() {} }" },
            { desc: "Acceso a miembros heredados:", code: "Avion boeing = new Avion();\nboeing.mover(); // Heredado\nboeing.volar(); // Propio" }
        ],
        exercises: [
            { instruction: "Crea una subclase 'Gerente' que herede de 'Empleado'.", codeChallenge: "public class Gerente extends Empleado { ... }" },
            { instruction: "Añade un método 'gestionar()' exclusivo para la clase 'Gerente'.", codeChallenge: "void gestionar() { ... }" }
        ]
    },
    {
        id: 4,
        ...
        examples: [
            { desc: "Referencia de superclase a objeto hijo:", code: "Animal miPerro = new Perro();\nmiPerro.hacerSonido(); // Ejecuta ladrido" },
            { desc: "Sobrescritura de métodos con @Override:", code: "@Override\nvoid mover() {\n    System.out.println(\"Nandando...\");\n}" }
        ],
        exercises: [
            { instruction: "Crea una clase 'Circulo' que sobrescriba 'dibujar()' de 'Forma'.", codeChallenge: "@Override\nvoid dibujar() { ... }" },
            { instruction: "Declara una lista de 'Forma' y añade un 'Circulo' y un 'Cuadrado'.", codeChallenge: "List<Forma> formas = new ArrayList<>();" }
        ]
    },
    {
        id: 5,
        ...
        examples: [
            { desc: "Definición de clase y método abstracto:", code: "abstract class Forma {\n    abstract double area();\n}" },
            { desc: "Implementación obligatoria en subclase:", code: "class Cuadrado extends Forma {\n    double area() { return lado * lado; }\n}" }
        ],
        exercises: [
            { instruction: "Crea una clase abstracta 'Instrumento' con el método 'tocar()'.", codeChallenge: "abstract class Instrumento { ... }" },
            { instruction: "Crea la clase 'Piano' que implemente el método 'tocar()'.", codeChallenge: "class Piano extends Instrumento { ... }" }
        ]
    },
    {
        id: 6,
        ...
        examples: [
            { desc: "Definición de una interfaz de comportamiento:", code: "interface Conectable {\n    void conectar();\n}" },
            { desc: "Implementación múltiple de interfaces:", code: "class SmartTV implements Conectable, Reproductor { ... }" }
        ],
        exercises: [
            { instruction: "Define una interfaz 'Nadador' con el método 'nadar()'.", codeChallenge: "interface Nadador { ... }" },
            { instruction: "Haz que la clase 'Delfin' implemente la interfaz 'Nadador'.", codeChallenge: "class Delfin implements Nadador { ... }" }
        ]
    },
    {
        id: 7,
        ...
        examples: [
            { desc: "Constructor con parámetros:", code: "public Persona(String nombre) {\n    this.nombre = nombre;\n}" },
            { desc: "Sobrecarga de constructores:", code: "public Persona() { this.nombre = \"Anonimo\"; }\npublic Persona(String n) { ... }" }
        ],
        exercises: [
            { instruction: "Crea un constructor para la clase 'Libro' que reciba el título.", codeChallenge: "public Libro(String t) { ... }" },
            { instruction: "Añade un constructor vacío que asigne valores por defecto.", codeChallenge: "public Libro() { ... }" }
        ]
    },
    {
        id: 8,
        ...
        examples: [
            { desc: "Resolución de ambigüedad de nombres:", code: "void setEdad(int edad) {\n    this.edad = edad; // this.edad es el atributo\n}" },
            { desc: "Llamada entre constructores de la misma clase:", code: "public Persona() {\n    this(\"Sin nombre\", 0); // Llama al otro constructor\n}" }
        ],
        exercises: [
            { instruction: "Usa 'this' para asignar un parámetro 'marca' al atributo 'marca'.", codeChallenge: "void updateMarca(String marca) { ... }" },
            { instruction: "Usa 'this()' para que un constructor sin parámetros llame a uno con parámetros.", codeChallenge: "public Coche() { ... }" }
        ]
    },
    {
        id: 9,
        ...
        examples: [
            { desc: "Invocación al constructor del padre:", code: "public Perro() {\n    super(\"Canino\"); // Llama al constructor de Animal\n}" },
            { desc: "Llamada a método sobrescrito:", code: "void comer() {\n    super.comer(); // Come como animal\n    System.out.println(\"Perro come croquetas\");\n}" }
        ],
        exercises: [
            { instruction: "Llama al constructor del padre desde la clase 'Moto'.", codeChallenge: "public Moto() { super(); }" },
            { instruction: "Accede al método 'apagar()' de la clase padre usando 'super'.", codeChallenge: "void apagar() { super.apagar(); }" }
        ]
    },
    {
        id: 10,
        ...
        examples: [
            { desc: "Diferencia entre public y private:", code: "public String nombre; // Visible en todo el proyecto\nprivate String id; // Solo visible en esta clase" },
            { desc: "Uso de protected para la herencia:", code: "protected double velocidadMax; // Visible para hijos" }
        ],
        exercises: [
            { instruction: "Declara un método privado llamado 'logInterno()'.", codeChallenge: "private void logInterno() { ... }" },
            { instruction: "Crea una variable protegida 'puntosVida' para una clase Guerrero.", codeChallenge: "protected int puntosVida;" }
        ]
    },
    {
        id: 11,
        ...
        examples: [
            { desc: "Variable compartida entre instancias:", code: "static int totalObjetos = 0;\npublic MiClase() { totalObjetos++; }" },
            { desc: "Método de utilidad estático:", code: "public static int sumar(int a, int b) { return a + b; }" }
        ],
        exercises: [
            { instruction: "Crea una constante estática PI en una clase 'Calculos'.", codeChallenge: "public static final double PI = 3.1416;" },
            { instruction: "Define un método estático que convierta de Celsius a Fahrenheit.", codeChallenge: "public static double toF(double c) { ... }" }
        ]
    },
    {
        id: 12,
        ...
        examples: [
            { desc: "Definición de constante inmutable:", code: "final int MAX_USUARIOS = 500;" },
            { desc: "Clase que impide la extensión:", code: "public final class Segura { ... }" }
        ],
        exercises: [
            { instruction: "Crea una variable 'final' para la gravedad terrestre (9.8).", codeChallenge: "final double GRAVEDAD = 9.8;" },
            { instruction: "Intenta declarar un método final 'validar()' en una clase Base.", codeChallenge: "public final void validar() { ... }" }
        ]
    },
    {
        id: 13,
        ...
        examples: [
            { desc: "Sobrecarga por tipo de parámetro:", code: "void mostrar(int i) { ... }\nvoid mostrar(String s) { ... }" },
            { desc: "Sobrecarga por cantidad de parámetros:", code: "int area(int lado) { ... }\nint area(int b, int h) { ... }" }
        ],
        exercises: [
            { instruction: "Sobrecarga el método 'buscar' para String y para int.", codeChallenge: "void buscar(String n) { }\nvoid buscar(int id) { }" },
            { instruction: "Crea dos versiones de un constructor usando sobrecarga.", codeChallenge: "public User() { }\npublic User(String n) { }" }
        ]
    },
    {
        id: 14,
        ...
        examples: [
            { desc: "Uso de ArrayList con polimorfismo:", code: "List<Persona> lista = new ArrayList<>();\nlista.add(new Persona(\"Ana\"));" },
            { desc: "Iteración sobre colección de objetos:", code: "for(Persona p : lista) {\n    p.saludar();\n}" }
        ],
        exercises: [
            { instruction: "Declara un ArrayList que solo acepte objetos de tipo 'Coche'.", codeChallenge: "ArrayList<Coche> garaje = new ArrayList<>();" },
            { instruction: "Usa un bucle para imprimir el nombre de todos los objetos en una lista.", codeChallenge: "for(Objeto o : lista) { ... }" }
        ]
    }
];
